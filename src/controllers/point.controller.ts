...
export const submitEvaluation = async (req: Request, res: Response) => {
  try {
    ...
    if (calculatedTotal > 100) calculatedTotal = 100;
    if (calculatedTotal < 0) calculatedTotal = 0;

    // Phân loại xếp loại theo khung điểm quy định
    let classification = 'Chưa đạt';
    if (calculatedTotal >= 90) classification = 'Xuất sắc';
    else if (calculatedTotal >= 80) classification = 'Tốt';
    else if (calculatedTotal >= 70) classification = 'Khá';
    else if (calculatedTotal >= 50) classification = 'Trung bình';

    const newEvaluation = await prisma.pointEvaluation.create({
      data: {
        semester,
        academicYear,
