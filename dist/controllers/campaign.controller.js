...
exports.updateDonationStatus = exports.getAllDonations = exports.donateItems = exports.getNearestCampaigns = exports.getFacultyLeaderboard = exports.getSystemStats = exports.getCampaignById = exports.remove = exports.update = exports.getRegistrations = exports.getAll = exports.create = void 0;
...
const getAll = async (req, res) => {
    try {
        ...
        if (type) {
            if (type === 'org')
                whereCondition.type = 'ORGANIZATION';
            else if (type === 'ind')
                whereCondition.type = 'INDIVIDUAL';
            else
        ...
        const [campaigns, totalCampaigns] = await Promise.all([
            ...
            prisma_1.default.campaign.count({ where: whereCondition })
        ]);
...
const getSystemStats = async (req, res) => {
    try {
        const [totalUsers, totalCampaigns, totalRegistrations, totalOrgs, totalIndiv] = await Promise.all([
            ...
            prisma_1.default.registration.count({ where: { status: 'APPROVED' } }),
            prisma_1.default.campaign.count({ where: { type: 'ORGANIZATION' } }),
            prisma_1.default.campaign.count({ where: { type: 'INDIVIDUAL' } }),
        ]);
        res.status(200).json({
            data: {
...
const getFacultyLeaderboard = async (req, res) => {
    try {
        ...
        const approvedRegistrations = await prisma_1.default.registration.findMany({
...
exports.getFacultyLeaderboard = getFacultyLeaderboard;
...
const getNearestCampaigns = async (req, res) => {
    try {
        ...
        const campaignsWithDistance = campaigns
            ...
            .map((camp) => {
            const distance = calculateDistance(user.lat, user.lng, camp.lat, camp.lng);
            return {
                ...camp,
                currentVolunteers: camp._count.registrations,
                distanceKm: distance
...
exports.getNearestCampaigns = getNearestCampaigns;
...
const donateItems = async (req, res) => {
    try {
        ...
        const newDonation = await prisma_1.default.itemDonation.create({
            data: {
                campaignId,
                userId,
                type,
...
exports.donateItems = donateItems;
// 2. Admin lấy toàn bộ danh sách quyên góp để quản lý
const getAllDonations = async (req, res) => {
    try {
        const donations = await prisma_1.default.itemDonation.findMany({
            include: {
                user: {
...
const updateDonationStatus = async (req, res) => {
    try {
        ...
        const updated = await prisma_1.default.itemDonation.update({
            where: { id: donationId },
            data: { status }
        });
